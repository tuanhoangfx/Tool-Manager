...
- Sheet: realtime parity theo account/cross-tab gần với 2FA/Cookie; chặn race xóa khi sync đang chạy; giữ banner offline ổn định khi fallback cache.
- 2FA: thêm `mail_recover`, tối ưu table/detail modal layout, single-line cells, import/backfill từ sheet/note cũ.
- Workspace cleanup: bỏ component/cell cũ không còn dùng, đồng bộ shell/table metadata liên quan.

### Verification

- `vitest run src/features/sheet`
- `vitest run src/features/sheet/sheet-sync-pending.test.ts src/features/sheet/sheet-sources-cloud.test.ts src/features/sheet/useSheetSourcesCloud.test.ts`

## 2026-06-24 - Sheet: keep offline warning stable during retries

- Version: `4.4.4`
- Type: Patch
- Product: P0020
- Prompt: Sửa lỗi banner offline của Sheet nháy liên tục khi đang fallback dữ liệu cache.

### Changes

- `SheetWorkspaceScreen`: giữ nguyên banner offline khi retry nền thất bại thay vì clear rồi set lại liên tục.
- Narrow load dependencies to source identity/fetch inputs, tránh reload grid chỉ vì metadata như `lastSyncedAt` đổi.

### Verification

- `vitest run src/features/sheet`
- Browser MCP: `/sheet` không còn nháy banner lỗi trong luồng verify hiện tại

## 2026-06-23 - Sheet: 2FA-style realtime parity + delete-during-sync fix

- Version: `4.4.3`
- Type: Patch
- Product: P0020
- Prompt: Sheet realtime cross-tab/account parity (2FA/Cookie) + fix xóa hiện lại khi sync đang chạy.

### Changes

- `sheet-sources-cross-tab.ts`, `useSheetRealtime.ts`: cross-tab BroadcastChannel + Supabase realtime reconcile (2FA/Cookie pattern).
- `useSheetSourcesCloud`: queue delete/reconcile while sync busy; focus + session reconcile; generation guard on boot sync.
- `sheet-sources-cloud`: skip pending-delete rows on cloud upsert (`selectSheetSourcesForCloudPush`); abort stale sync loops.
- `sheet-sources`: cross-tab notify on local changes; `saveSheetSources` filters pending deletes.

### Verification

- `vitest run src/features/sheet`

## 2026-06-23 - Sheet: queue delete while cloud sync is busy

- Version: `4.4.2`
- Type: Patch
- Product: P0020
- Prompt: Sheet vẫn tự hiện lại sau khi xóa vì delete bị rớt nếu cloud sync đang chạy.
- Status: Draft

### Changes

- `useSheetSourcesCloud`: queue `delete` / `upsert` events while sync or realtime reconcile is busy, then drain after sync completes.
- `useSheetSourcesCloud.test.ts`: cover queue merge behavior so latest action for a source is preserved without dropping deletes.

### Verification

- `vitest run src/features/sheet/sheet-sync-pending.test.ts src/features/sheet/sheet-sources-cloud.test.ts src/features/sheet/useSheetSourcesCloud.test.ts`

## 2026-06-23 - 2FA detail modal: fill frame, align credential rows

- Version: `4.3.52`
- Timestamp: 2026-06-23 12:45 (UTC+7)
- Type: Patch
- Product: P0020

### Changes

- Account detail modal: Credentials / Note / Change log stretch to fill modal body (no empty band at bottom).
- Form grid uses fixed label width so inputs align across rows.
- Row layout: Service · Browser · Mail Recover; **Account · Password · Secret** always on one row (tablet+).

### Verification

- CSS/layout change in `twofa-account-detail-modal.css` + `TwofaAccountDetailModal.tsx`

## 2026-06-23 - 2FA: Mail Recover column, table single-line, modal layout

- Version: `4.3.51`
- Timestamp: 2026-06-23 12:20 (UTC+7)
- Type: Patch
- Product: P0020

### Changes

- Remove Mail/Services preset toolbar (All | Mail | Services).
- Add **Mail Recover** column + `mail_recover` vault field; sheet import maps **Mail** column; backfill **684** rows from legacy `Mail:` note lines.
- Table: Note hidden by default; cells single-line ellipsis (no row height drift).
- Account detail modal: credentials grouped 3+3; meta row Created/Updated/Vault ID.

### Verification

- `vitest run src/features/twofa` — 95 passed
- Migration `20260623140000_twofa_accounts_mail_recover.sql` applied
- `backfill-twofa-mail-recover-from-note.mjs --apply`

## 2026-06-23 - Sheet: fix deleted source reappearing after cloud sync

- Version: `4.4.1`
- Type: Patch
- Product: P0020
- Prompt: Sheet — sau khi xóa, sheet tự hiện lại sau 1–2s (pattern pending delete từ 2FA).
- Status: Draft

### Changes

- `sheet-sync-pending.ts`: mute in-flight local deletes by id + dedupe key until cloud tombstone ack.
- `removeSheetSource`, `sheet-sources-cloud`, `useSheetSourcesCloud`: filter pending deletes on merge/reconcile/sync; bump sync generation on delete.

### Verification

- `vitest run sheet-sync-pending.test.ts sheet-sources-cloud.test.ts`

## 2026-06-23 - 2FA: Mail/Services preset, large-directory page cap, vault purge

- Version: `4.3.49`
- Timestamp: 2026-06-23 11:30 (UTC+7)
- Type: Patch
- Product: P0020

### Changes

- Toolbar preset filter **All | Mail | Services** (`2cat` URL param) — same vault, no screen split; counts reflect current search/filters.
- Large directories (>500 visible rows): cap table + card pager at 50 rows/page (Hub parity — no custom virtual table bypass).
- Cloud vault purge: tombstoned **34** duplicate rows (6130 → 6096 active); open 2FA tab once to sync local cache.

### Verification

- `vitest run src/features/twofa` — 99 passed
- `purge-twofa-vault-duplicates.mjs --dry-run` + `--apply`

## 2026-06-23 - 2FA perf: remove vault toolbar chips, large-vault fast paths

- Version: `4.3.48`
- Timestamp: 2026-06-23 03:15 (UTC+7)
- Type: Patch
- Product: P0020

### Changes

- Remove `Connecting vault…` / `vault N` trailing chips from 2FA directory toolbar (and auto full-sync retry after 10s idle).
- Large vault (>2k): skip faceted filter option counts, skip log text in search haystack, skip local dedupe on load when migration done, slower localStorage persist, throttle window-focus cloud reconcile to 5 min.
- Defer search query via `useDeferredValue` so typing stays responsive with ~6k rows.
