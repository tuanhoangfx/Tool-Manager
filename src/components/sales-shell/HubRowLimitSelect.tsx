...
import { DEFAULT_HUB_ROW_LIMIT, LIMIT_OPTIONS, patchHubListPrefs } from "../../lib/url-prefs";

/** Same control as P0004-Tool-Hub HubListPage filter toolbar. */
export function HubRowLimitSelect({ value }: { value: number }) {
  ...
  function pick(next: number) {
    patchHubListPrefs({ limit: next === DEFAULT_HUB_ROW_LIMIT ? null : String(next) });
    setOpen(false);
  }
