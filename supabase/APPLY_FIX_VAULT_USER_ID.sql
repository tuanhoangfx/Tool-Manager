...
--   1) RLS policies on note_cookie_vault were missing (RLS enabled with 0 policies
--      → Postgres denies all authenticated access by default). Tool/extension saw
--      "No vault yet" even when rows existed.
--   2) note_vault_upsert used
--        `select user_id into v_uid from note_verify_sync_pass(...)`
--      which does not unpack the composite (`public.notes`) return value in this
--      context → v_uid ended up NULL or as the note_id, causing FK violations and
--      mismatched user_id.
--
-- Paste into Supabase Dashboard → SQL Editor → Run.

-- 1) Backfill any rows that ended up with stale or NULL user_id.
...
-- 3) Replace note_vault_upsert: read user_id directly from notes instead of
--    going through note_verify_sync_pass (composite return pitfall).
